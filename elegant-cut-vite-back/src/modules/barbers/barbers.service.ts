...
export class BarbersService {
  ...
  async crearBarbero(createBarberDto: CreateBarberDto) {
    ...
    const nuevoBarbero = await this.barbersRepo.createBarberWithPortfolio(
      userData,
      portfolioData,
    );
  ...
  async update(id: number, data: any) {
    ...
    if (Object.keys(portafolioData).length > 0) {
      const portafolioExistente =
        await this.barbersRepo.findPortfolioByUserId(id);

      if (portafolioExistente) {
        await this.barbersRepo.updatePortfolio(
          portafolioExistente.id_portafolio,
          portafolioData,
        );
      } else {
  ...
  private mapBarberWithRating(barber: any) {
    ...
    const sum = resenas.reduce(
      (acc: number, r: any) => acc + r.calificacion,
      0,
    );
    ...
    const portfolio = Array.isArray(barber.portafolios)
      ? barber.portafolios[0]
      : barber.portafolios;
    if (portfolio) {
